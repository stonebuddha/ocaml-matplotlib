...
let call_plot_func p func ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys =
  let keywords =
    List.filter_opt
      ...
      ; Option.map marker ~f:(fun m -> "marker", Py.String.of_string (String.of_char m))
...
let plot p ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys =
  call_plot_func p `plot ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys

let semilogy p ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys =
  call_plot_func p `semilogy ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys

let semilogx p ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys =
  call_plot_func p `semilogx ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys

let loglog p ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys =
  call_plot_func p `loglog ?label ?color ?linewidth ?linestyle ?marker ?alpha ?xs ys
...
let scatter p ?label ?s ?c ?marker ?alpha ?linewidths xys =
  let keywords =
    List.filter_opt
...
let legend p ?labels ?loc ?framealpha () =
  let keywords =
    List.filter_opt
      ...
      ; Option.map framealpha ~f:(fun f -> "framealpha", Py.Float.of_float f)
      ]
  in
